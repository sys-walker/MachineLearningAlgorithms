...
class Kmeans:
    ...
    def fit(self, rows, restarting_policies=1):
        ...
        if self.verbose_mode and restarting_policies > 1:
            print("\n=========== Start restarting policies", restarting_policies, "times =========== ")
        for i in range(restarting_policies):
            self.centroids = list()
            ...
            if self.verbose_mode and restarting_policies > 1:
                print("Local inertia_: ", inertia_)
                print("local bestmatches: ", lastmatches)
                print("Local Centroids: ", self.centroids)
            if inertia_ > inertia_min_:
                inertia_min_ = inertia_
                lastmatches_best = lastmatches
            if self.verbose_mode and restarting_policies > 1 and i < restarting_policies - 1:
                print("\n--------- restarting policies ------------")

        if self.verbose_mode and restarting_policies > 1:
            print("\n=========== End restarting policies =========== ")
            print("\n+++++++++++++++ Best results ++++++++++++++++++ ")
            print("inertia_: ", inertia_min_)
            print("bestmatches: ", lastmatches_best)
...
def total_distance_function(rows, k_inicial=2, k_final=10, use_range=True, restarting_policies=1, verbose=False,
                            verbose_k_means=False):
    """ Creates a table a relation with inertia and cluster """
    inertias = []
    if verbose and restarting_policies > 1:
        print("Kmeans using restarting policies\n")
...
if __name__ == '__main__':
    ...
    total_distance_function(data, k_inicial=2, k_final=8, use_range=True, restarting_policies=3, verbose=True)

    # data = [[2, 4],[3, 5],[3, 2],[5, 2],[5, 4],[7, 3],[7, 8],[8, 4]]
    # kmeans = Kmeans(k=9, distance=euclidean_squared,use_range=True, max_iters=1000,verbose=True)
